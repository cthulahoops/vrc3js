...
export function parseVerificationFixture(value: unknown): VerificationFixture {
  ...
  if (!Array.isArray(fixture.entities)) throw new Error('fixture.entities must be an array');
  const entities = fixture.entities.map((entity, index) => {
    const sanitized = sanitizeEntity(entity);
    if (!sanitized) throw new Error(`fixture.entities[${index}] is not a valid protocol entity`);
    return sanitized;
  });
  ...
  const fov = camera.fov == null ? undefined : finite(camera.fov, 'fixture.camera.fov');
  if (fov != null && (fov <= 0 || fov >= 180)) {
    throw new Error('fixture.camera.fov must be between 0 and 180 degrees');
  }

  if (typeof fixture.time !== 'string') throw new Error('fixture.time must be an ISO date string');
  const time = new Date(fixture.time);
  if (!Number.isFinite(time.getTime())) throw new Error('fixture.time must be a valid ISO date string');
  ...
  return {
    ...
    camera: {
      ...
      orientation: {
        ...
        roll: orientation.roll == null ? 0 : finite(orientation.roll, 'fixture.camera.orientation.roll'),
      },
      ...(fov == null ? {} : { fov }),
    },
    time,
