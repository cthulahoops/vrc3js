...
test('parses a deterministic renderer fixture', () => {
  ...
  assert.deepEqual(fixture.camera.orientation, { yaw: 0.5, pitch: -0.1, roll: 0 });
...
test('rejects invalid fixtures before they reach the renderer', () => {
  assert.throws(() => parseVerificationFixture({
    entities: [{ id: 'desk-1', type: 'Desk', pos: { x: Infinity, y: 2 } }],
    camera: { position: { x: 0, y: 1, z: 2 }, orientation: { yaw: 0, pitch: 0 } },
    time: '2021-06-20T16:00:00Z',
  }), /entities\[0\]/);
  assert.throws(() => parseVerificationFixture({
    entities: [],
