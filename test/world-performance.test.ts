...
import type {
  EntityHandle,
  VirtualRcRenderer as VirtualRcRendererType,
} from "../src/world.js";
...
function wall(
  id: string,
  x = 0,
  overrides: Partial<Omit<WallEntity, "id" | "type">> = {},
): WallEntity {
  return {
    id,
    type: "Wall",
...
function firstBatch(
  renderer: VirtualRcRendererType,
  object: EntityHandle | undefined,
): InstanceBatch {
  assert.ok(object);
...
test("snapshot replacement reconciles by id instead of rebuilding the world", () => {
  ...
  assert.equal(
    renderer.instanceBatches.locations.has(
      removed!.userData.components![0]!.key,
    ),
    false,
  );
  assert.equal(renderer.entities.has("add"), true);
  assert.deepEqual([...renderer.entities.keys()].sort(), [
    "add",
    "keep",
    "move",
  ]);
...
test("an update that becomes non-renderable removes its previous object", () => {
  ...
  assert.equal(
    renderer.instanceBatches.locations.has(
      previous!.userData.components![0]!.key,
    ),
    false,
  );
});
...
test("shared resources live until renderer disposal, not individual entity deletion", () => {
  ...
  batch.material.addEventListener("dispose", () => {
    disposeCounts.material += 1;
  });
  batch.material.map.addEventListener("dispose", () => {
...
test("a representative update stream keeps renderer resource cardinality bounded", () => {
  ...
  assert.ok(
    [...renderer.entities].every(
      ([id, object]) => object === initialObjects.get(id),
    ),
  );
...
test("desks collapse all components and colors into one shared draw batch", () => {
  ...
  assert.equal(renderer.instanceBatches.batches.size, 1);
  assert.deepEqual(
    [...renderer.instanceBatches.batches.values()].map((batch) => batch.size),
    [10],
  );
  assert.equal(
    scene.children.filter((child) => child instanceof THREE.InstancedMesh)
...
test("different cube dimensions share a material batch through matrix scaling", () => {
  ...
  renderer.setEntityComponents(handle, [
    ...
    renderer.cube(new THREE.Vector3(0.2, 0.4, 0.6), "#123456"),
  ]);
...
test("fixture world batches its nineteen cube components into thirteen draws", () => {
  ...
  const componentCount = [...renderer.entities.values()].reduce(
